...
export interface CartItem {
  ...
  quantity: number;
  skuId?: string;
  skuLabel?: string; // Display label for selected variants
}
...
export const useStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      addItem: (product, quantity = 1, skuId, skuLabel) => {
        const items = get().items;
        // If product has variants, check for same SKU; otherwise check for same product
        const existingItem = items.find(item => 
          skuId 
            ? item.skuId === skuId 
            : item.productId === product.id && !item.skuId
        );
        
        if (existingItem) {
          set({
            items: items.map(item =>
              (skuId ? item.skuId === skuId : item.productId === product.id && !item.skuId)
