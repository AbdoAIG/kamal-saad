...
// Simplified category type that works with both server-serialized and API data
...
// Product type that accepts both server-serialized (ISO string dates) and API-returned data
...
export function HomePageClient({
  ...
  categoryFilter: serverCategoryFilter,
}: HomePageClientProps) {
  ...
  const [isLoading, setIsLoading] = useState(initialProducts.length === 0 && !isEmpty);
  ...
  // Track whether client-side search/category params differ from server-provided ones
  const isClientParamChange = searchQuery !== serverSearchQuery || categoryFilter !== serverCategoryFilter;

  // Fetch data from API (used when search/category changes client-side)
  ...
  useEffect(() => {
    // Only re-fetch from API if the client-side search/category params
    // differ from the server-provided ones (i.e., user navigated client-side)
    if (isClientParamChange) {
      setIsLoading(true);
      setPage(1);
      setProducts([]);
      fetchInitialData();
    }
  }, [searchQuery, categoryFilter, fetchInitialData, isClientParamChange]);
