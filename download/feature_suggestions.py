...
story.append(Paragraph("تقرير المقترحات", title_style))
story.append(Paragraph("مميزات إضافية لموقع كمال سعد للقرطاسية", subtitle_style))
story.append(Spacer(1, 30))
story.append(Paragraph("تحليل شامل للمميزات الناقصة والتحسينات المقترحة", subtitle_style))
story.append(Spacer(1, 50))

# Summary stats
summary_data = [
    ['المجموع', 'متوسطة', 'عالية'],
    ['35', '15', '20'],
]
...
story.append(Paragraph("المميزات عالية الأولوية", section_style))
story.append(Spacer(1, 10))

high_priority_features = [
    ("نظام التقييمات والمراجعات", "تمكين العملاء من كتابة تقييمات ومراجعات للمنتجات مع إمكانية إضافة صور. هذا يعزز الثقة ويساعد العملاء الآخرين في اتخاذ قرارات الشراء. يجب أن يتضمن النظام تصنيف بالنجوم من 1 إلى 5، ونص المراجعة، وإمكانية الرد من قبل المتجر."),
...
for title, desc in high_priority_features:
    story.append(Paragraph(f"• {title}", feature_title_style))
...
story.append(Paragraph("المميزات متوسطة الأولوية", section_style))
story.append(Spacer(1, 10))
...
medium_priority_features = [
    ...
    ("المنتجات المُشاهدة مؤخراً", "عرض قائمة بالمنتجات التي شاهدها العميل مؤخراً في الصفحة الرئيسية أو في الشريط الجانبي، مما يسهل عليه العودة للمنتجات التي أثارته اهتمامه."),
    ...
    ("صفحات قانونية (سياسة الخصوصية، الشروط والأحكام)", "إضافة الصفحات القانونية الضرورية مثل سياسة الخصوصية، شروط الاستخدام، وسياسة الإرجاع لضمان الشفافية والحماية القانونية."),
...
story.append(Paragraph("مميزات إضافية مقترحة", section_style))
story.append(Spacer(1, 10))
...
additional_features = [
    ...
    ("معاينة الواقع المعزز AR", "إضافة تقنية الواقع المعزز لمعاينة بعض المنتجات مثل الحقائب والأدوات المكتبية في بيئة حقيقية قبل الشراء."),
    ("نظام القوائم المخصصة", "إمكانية إنشاء قوائم أمنيات متعددة (مثلاً: قائمة المدرسة، قائمة المكتب، قائمة الهدايا) بدلاً من قائمة مفضلة واحدة."),
    ("الاشتراك في النشرة البريدية", "نظام اشتراك متكامل للنشرة البريدية مع إرسال رسائل بريد إلكتروني تلقائية للعروض والمنتجات الجديدة."),
    ("تقسيط مخصص", "التكامل مع أنظمة التقسيط المحلية مثل فالي وتوفر خيارات تقسيط إضافية من بنوك مختلفة."),
]
...
story.append(Paragraph("التحسينات التقنية", section_style))
story.append(Spacer(1, 10))
...
# Summary table
story.append(Paragraph("جدول ملخص الأولويات", section_style))
story.append(Spacer(1, 10))
